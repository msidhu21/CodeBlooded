...
class ItemRepo:
    ...
    def search(self, *, q: str | None, category: str | None, available: bool | None, page: int, size: int) -> List[Item]:
        stmt = select(Item)
        if q:
            like = f"%{q}%"
            stmt = stmt.where((Item.name.like(like)) | (Item.description.like(like)))
        if category:
            stmt = stmt.where(Item.category == category)
        if available is not None:
            stmt = stmt.where(Item.available == available)
